...
app.get('/isBusLoggedIn', (request, response) => {
    ...
    if (!request.session.busData) {
        ...
        con.query(
            "SELECT * FROM buses WHERE id=?", [request.session.busData.id],
...
app.get('/busLogOut', (request, response) => {
    ...
    if (request.session.busData) {
        con.query("UPDATE buses SET auth_token = ? WHERE id = ?", [null, request.session.busData.id],
        ...
        if (results.length === 0) {
            ...
            bcrypt.compare(req.body.password, password, function(err, res) {
                ...
                if (res === true) {
                    ...
                    con.query("UPDATE buses SET auth_token = ? WHERE id = ?", [results[0]['id'].toString(), results[0]['id']],
...
busesSockets.on('connection', function(socket) {
    //bagam autobuzele in buses dupa ce se logheaza
    socket.on('storeClientInfo', function(data) {
    ...
    socket.on('bus-new-location', function(data) {
        console.log(data);
        con.query('UPDATE buses SET current_location_lat = ?, current_location_lng = ?, speed = ?, updated_at = ? WHERE id = ?', [data.currentLocationLat, data.currentLocationLng, data.speed, data.currentTime, data.busId],
            function(error, results, fields) {
                console.log(results);
            });
    });
